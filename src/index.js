...
${tweet.text}
`
        tweet.media = tweet.media.filter(e => e.type === 'video' && !e.media.indexOf(".m3u8?"))
        if (tweet.media.length !== 0) {
            // we need to double check if type are matched: video only or image only
            let mediaGroup = tweet.media

            const v = tweet.media.filter(e => e.type === 'video' && !e.media.indexOf(".m3u8?"))
            ...
            await retry(async (err, num) => {
                ...
                retries: retryTimes
            }).catch(e => consola.error(`${tweet.id} send failed after 5 retries`+e))
            continue
        }
